...
namespace BuildReportTool.Window.Screen
{
	public partial class AssetList
	{
		...
		{
			return list[idx].AssetPath.EndsWith(fileTypeInIdx, StringComparison.OrdinalIgnoreCase) &&
			       (idx < list.Count - 1) &&
			       list[idx + 1].AssetPath.EndsWith(fileTypeInNext, StringComparison.OrdinalIgnoreCase);
		}
			...
			BuildInfo buildReportToDisplay, AssetDependencies assetDependencies)
		{
			...
			if (dependencies.ContainsKey(selectedAsset.Name))
			{
				selectedAssetDependencies = dependencies[selectedAsset.Name];
				...
				switch (_selectedAssetUsageDisplayIdx)
				{
					...
					case ASSET_USAGE_DISPLAY_DIRECT:
						    ...
						    _assetUserCrumbActiveIdx < _assetUserCrumbs.Count)
						{
							...
							if (dependencies.ContainsKey(activeAssetUserCrumb))
							{
								countToUse = dependencies[activeAssetUserCrumb].Users.Count;
							}
						}
			...
			if (_showAssetUsagesList)
			{
				...
				switch (_selectedAssetUsageDisplayIdx)
				{
					...
					case ASSET_USAGE_DISPLAY_DIRECT:
						...
						if (dependencies.ContainsKey(assetToShowDirectUsersOf))
						{
							users = dependencies[assetToShowDirectUsersOf].Users;
						}
		...
		void DrawAssetUsageAncestry(Rect position, float availableWidth, BuildInfo buildReportToDisplay, AssetDependencies assetDependencies)
		{
			...
			for (int n = 0, len = _assetUsageAncestry.Count; n < len; ++n)
			{
				...
				var isAssetUsedByScript = (n < len - 1) && _assetUsageAncestry[n + 1].AssetPath.EndsWith(".cs", StringComparison.OrdinalIgnoreCase);
			...
			for (int n = 0, len = list.Count; n < len; ++n)
			{
				...
				if (BuildReportTool.Options.IsAssetUsageLabelTypeOnVerbose)
				{
					if (n == 0)
					{
						...
						else if ((n < len - 1) && list[n + 1].AssetPath.EndsWith(".cs", StringComparison.OrdinalIgnoreCase))
						{
							widthToAdd = assetLabelInBetweenStyle.CalcSize(AssetUsageIsUsedAsDefaultValueByLabel).x;
						}
						else
